...
static void ws_build_cnv_buffer(uint8_t *buf, uint32_t len)
{
    ...
    for (uint32_t i = 3; i < len; i++) {			// ARREGLO HARD-CODEADO i=3, debería ser i=0 pero segundo frame que se prende aparece shifteado 1 led ose 3 by
        uint8_t byte = buf[i];
